...
int main(int argc, char* argv[]) {
  ...
  char* yuv_buf = new char[framesize];
	...
	int FrameCount = ps / framesize; 
	LOG(INFO) << "[Total] Frame number: " << FrameCount; 

  fin.clear();
  fin.seekg(0, std::ios_base::beg);
  unsigned long long start_time = 0, end_time = 0;
  unsigned long long read_time = 0, detect_time = 0;
  unsigned long long read_average_time = 0, detect_average_time = 0;
  ...
  if(FLAGS_output_image) {
      ...
      std::string output_path = FLAGS_output_folder + "/" + input_name;
      LOG(INFO) << output_path;
      writer.open(output_path, cv::VideoWriter::fourcc('M', 'J', 'P', 'G'), 25, cv::Size(image_width, image_height), true);
    }
    ...
    fin.read(yuv_buf, framesize);
		cv::Mat yuv_img(image_height * 3 / 2, image_width, CV_8UC1);
		memcpy(yuv_img.data, yuv_buf, framesize*sizeof(char));
		cv::Mat rgb_img;
    ...
    LOG(INFO) << "\033[0;31mFrame: " << i << ", Read time: " << read_time << "ms, Detect time: " << detect_time << " ms. \033[0;39m";
	}

  LOG(INFO) << "\033[0;31mRead average time = " << read_average_time / FrameCount << "ms, Detect average time = " << detect_average_time / FrameCount << "ms. \033[0;39m";
  fin.close();
