...
def train(args):
    ...
    time_log_file = os.path.join(cfg.general.save_dir, 'logging', 'train_time_log.txt')
    logger_time = setup_logger(time_log_file, 'kws_time')
    ...
    train_dataloader, len_train_dataset = generate_dataset(cfg, 'training', args.training_mode)
    if cfg.general.is_test:
        eval_validation_dataloader = generate_test_dataset(cfg, 'validation', training_mode=args.training_mode)
    ...
    # profiler
    profiler = Profiler(summarize_every=cfg.train.show_log, disabled=False, logger=logger_time)
    ...
    for i in range(batch_number):
        ...
        # Parameter update
        optimizer.step()
        update_scheduler(cfg, scheduler, bool_updata_epoch=(last_save_epoch != epoch_idx))
        ...
        accuracy = torch.eq(pred_y, labels).sum().float() / labels.size(0)                     # 方式一：更高效
        # accuracy = torch.sum((pred_y == labels).type(torch.FloatTensor)) / labels.size(0)    # 方式二
