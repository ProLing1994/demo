...
def speed_volume_disturbution(config_file, speed_list, volume_list):
    ...
    for speed_idx in tqdm(range(len(speed_list))):
        for volume_idx in tqdm(range(len(volume_list))):
            output_dir = os.path.join(cfg.general.data_dir, '../{}_augumentation'.format(os.path.basename(cfg.general.data_dir)), positive_label + "_speed_{}_volume_{}".format("_".join(speed_list[speed_idx].split('.')), "_".join(volume_list[volume_idx].split('.'))))
