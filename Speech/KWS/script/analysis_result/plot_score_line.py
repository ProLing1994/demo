...
def plot_line(plot_x, plot_y_src, plot_y_pst):
...
def show_score_line(src_csv, pst_csv, label):
    ...
    for idx in range(len(src_list)):
        ...
        plot_y_src = np.zeros(plot_x.shape)
        plot_y_pst = np.zeros(plot_x.shape)
        for x_idx in range(plot_x.shape[0]):
            if plot_x[x_idx] > src_list[idx]['start_time'] and plot_x[x_idx] < src_list[idx]['end_time'] and src_list[idx]['label'] == label:
                plot_y_src[x_idx] = 1
        ...
        plt.figure()
        plot_line(plot_x, plot_y_src, plot_y_pst)
        # plt.show()
