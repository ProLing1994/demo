...
class SpeechResModel(nn.Module):
    ...
    def __init__(self, num_classes, image_height, image_weidth, width_multiplier=1.5):
        ...
        # init
        self.planes = [16, 24, 32, 48, 72]
        self.width_multiplier = width_multiplier
        self.first_conv_kernel = (3, 1)
        self.last_conv_kernel = (13, 1)
        self.in_planes = int(self.planes[0] * self.width_multiplier)

        self.conv0 = nn.Conv2d(1, image_weidth, kernel_size=(1, 40), stride=1, padding=(0, 0), bias=False)
        self.bn0 = nn.BatchNorm2d(image_weidth)
        ...
        self.conv2 = nn.Conv2d(int(self.planes[4] * self.width_multiplier), num_classes, kernel_size=self.last_conv_kernel,
    ...
    def forward(self, x):
        out = self.relu(self.bn0(self.conv0(x)))                        # shape: (batch, 1, 201, 40) -> shape: (batch, 40, 201, 1)
        out = self.relu(self.conv1(out))                                  # shape: (batch, 40, 201, 1) -> shape: (batch, 24, 201, 1)
        out = self.layer1_1(out)                                        # shape: (batch, 24, 201, 1) -> shape: (batch, 36, 101, 1)
        ...
        out = self.layer4_1(out)                                        # shape: (batch, 72, 26, 1) -> shape: (batch, 108, 13, 1)
        out = self.layer4_2(out)                                        # shape: (batch, 108, 13, 1) -> shape: (batch, 108, 13, 1)
