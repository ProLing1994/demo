...
class SpeechResModel(nn.Module):
    ...
    def __init__(self, num_classes, image_height, image_weidth, width_multiplier=1.5):
        ...
        self.conv1 = nn.Conv2d(1, self.in_planes, kernel_size=self.first_conv_kernel,
    ...
    def forward(self, x):
        out = self.relu(self.conv1(x))                                  # shape: (batch, 1, 101, 1) -> shape: (batch, 24, 101, 1)
        out = self.layer1_1(out)                                        # shape: (batch, 24, 101, 1) -> shape: (batch, 36, 51, 1)
        out = self.layer1_2(out)                                        # shape: (batch, 36, 51, 1) -> shape: (batch, 36, 51, 1)
        out = self.layer2_1(out)                                        # shape: (batch, 36, 51, 1) -> shape: (batch, 48, 26, 1)
        out = self.layer2_2(out)                                        # shape: (batch, 48, 26, 1) -> shape: (batch, 48, 26, 1)
        out = self.layer3_1(out)                                        # shape: (batch, 48, 26, 1) -> shape: (batch, 72, 13, 1)
        out = self.layer3_2(out)                                        # shape: (batch, 72, 13, 1) -> shape: (batch, 72, 13, 1)
        return out