...
class SpeechResModel(nn.Module):
    ...
    def __init__(self, num_classes, image_height, image_weidth, width_multiplier=2.0):
        super().__init__()
        ...
        self.linear = nn.Linear(int(self.planes[4] * self.width_multiplier), num_classes)
    ...
    def forward(self, x):
        ...
        out = self.relu(self.conv1(x))                                  # shape: (batch, 40, 201, 1) -> shape: (batch, 32, 201, 1)
        out = self.layer1_1(out)                                        # shape: (batch, 32, 201, 1) -> shape: (batch, 48, 101, 1)
        out = self.layer1_2(out)                                        # shape: (batch, 48, 101, 1) -> shape: (batch, 48, 101, 1)
        out = self.layer2_1(out)                                        # shape: (batch, 48, 101, 1) -> shape: (batch, 64, 51, 1)
        out = self.layer2_2(out)                                        # shape: (batch, 64, 51, 1) -> shape: (batch, 64, 51, 1)
        out = self.layer3_1(out)                                        # shape: (batch, 64, 51, 1) -> shape: (batch, 96, 26, 1)
        out = self.layer3_2(out)                                        # shape: (batch, 96, 26, 1) -> shape: (batch, 96, 26, 1)
        out = self.layer4_1(out)                                        # shape: (batch, 96, 26, 1) -> shape: (batch, 128, 13, 1)
        out = self.layer4_2(out)                                        # shape: (batch, 128, 13, 1) -> shape: (batch, 128, 13, 1)
        out = out.view(out.size(0), out.size(1), -1)                    # shape: (batch, 128, 13, 1) ->  # shape: (batch, 128, 13)
        out = torch.mean(out, 2)                                        # shape: (batch, 128, 13) ->  # shape: (batch, 128)
