            ...
            for equipment_idx in range(len(args.equipment_name_list)):
                ...
                for line_idx in range(len(lines)):
                    ...
                    if line.split(':')[-1].strip() == segment_label:
                        audio_segments.append([max(0, int(line.split(':')[0].split('~')[0]) + expansion_rate_front * segment_sample_rate), int(line.split(':')[0].split('~')[1]) + expansion_rate_back * segment_sample_rate, line_idx])
                f.close()
...
def main():
    ...
    parser.add_argument('--input_folder', type=str, default="/mnt/huanyuan/data/speech/kws/xiaoan_dataset/original_dataset/XiaoAnXiaoAn_05132021/处理音频_0422/") 
    parser.add_argument('--output_folder', type=str, default="/mnt/huanyuan/data/speech/kws/xiaoan_dataset/original_dataset/XiaoAnXiaoAn_05132021/xiaoan_0422/") 
    parser.add_argument('--output_format', type=str, default="RM_KWS_XIAOAN_xiaoan_S{:0>3d}M0D{}{}T{:0>3d}.wav")
    parser.add_argument('--id_name_csv', type=str, default="/mnt/huanyuan/data/speech/kws/xiaoan_dataset/original_dataset/唤醒词记录.csv") 
