...
int main(int argc, char **argv) {
	...
	// ASR::GetMelFilter(mel_filter, window_len, sample_rate, feature_freq, 48);
	ASR::GetMelFilter(&mel_filter, window_len, sample_rate, feature_freq);
    ...
    for (unsigned int idx = 0; idx < audio_names.size(); idx++) {
        ...
        for(int times = 0; times < windows_times; times ++) {
            ...
            ASR::GetFrequencyFeature(audio_data, window_size_samples, &speech_feature, window_len, sample_rate, time_step_ms);
	        ASR::GetMFSC(speech_feature, mel_filter, &mfsc_feature, feature_freq);
