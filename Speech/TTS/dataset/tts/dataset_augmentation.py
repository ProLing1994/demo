...
def dataset_augmentation_longer_senteces(cfg, text, wav, data_by_spk, lmdb_dict):
    ...
    for find_idx in range(len(find_data_by_spk_index)):
        ...
        dataset_name = data_by_spk.loc[find_data_index, 'dataset']
        ...
        # mel
        find_wav = read_audio_lmdb(lmdb_dict[dataset_name], str(data_name))
...
def dataset_augmentation_longer_senteces_hdf5(cfg, text, wav, data_by_spk):
    # init
    longer_senteces_frequency = cfg.dataset.augmentation.longer_senteces_frequency 
    longer_senteces_length = cfg.dataset.augmentation.longer_senteces_length
    longer_senteces_weight = cfg.dataset.augmentation.longer_senteces_weight
    ...
    for find_idx in range(len(find_data_by_spk_index)):
        ...
        # mel
        find_wav_path = os.path.join(cfg.general.data_dir, 'dataset_audio_hdf5', dataset_name, data_name.split('.')[0] + '.h5')
        find_wav = read_hdf5(find_wav_path, "wave").T.astype(np.float32)
