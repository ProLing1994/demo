...
def forward():
    ...
    img = cv2.imread("/phzhang/mmdetection-master-zhiguan/tools/deployment/pytorch2caffe/test.jpg")
    ...
    cls_preds.append(caffe_blobs["cat_blob2"].data[0][...])
    # print(caffe_blobs["cat_blob2"].data[0][...].shape)
    bbox_preds.append(caffe_blobs["relu_blob49"].data[0][...].flatten())

    cls_preds.append(caffe_blobs["cat_blob3"].data[0][...])
    bbox_preds.append(caffe_blobs["relu_blob50"].data[0][...].flatten())

    cls_preds.append(caffe_blobs["cat_blob4"].data[0][...])

    bbox_preds.append(caffe_blobs["relu_blob51"].data[0][...].flatten())

    featuremap_sizes = [40, 20, 10]
    featuremap_scales = [0.5682, 0.5302, 0.5519]

    for cls_pred, bbox_pred, featuremap_size, scale_ in zip(cls_preds, bbox_preds, featuremap_sizes, featuremap_scales):
        cls_pred = torch.Tensor(cls_pred)
        bbox_pred = torch.Tensor(bbox_pred).reshape(4, featuremap_size * featuremap_size).permute(1, 0)

        cls_pred = torch.max(cls_pred, 0).values.flatten()
