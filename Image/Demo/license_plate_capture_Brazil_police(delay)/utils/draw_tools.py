...
def draw_bbox_info(img, bbox_info, capture_list=None, mode='xywh'):
    ...
    for key, capture_draw_idy in capture_draw_container.items():
        ...
        # text = "{}_{}_{}".format( capture_draw_idy['id'], capture_draw_idy['column'], capture_draw_idy['num'] )
        text_size = cv2.getTextSize(text, 0, 3, 2)
        cv2.rectangle(img, (x, y), (x + text_size[0][0] + 2 * text_size[1], y + text_size[0][1] + text_size[1]), (255,255,255), -1)
    ...
    for idx in range(len(bbox_info)):
        ...
        if not capture_bool:
            img = cv_plot_rectangle(img, bbox_info_idx['loc'], mode=mode, color=color_dict["plate"], thickness=3)
            ...
            # img = cv2.putText(img, "{}_{}_{}_{}_{:.2f}".format( bbox_info_idx['id'], bbox_info_idx['column'], bbox_info_idx['kind'], bbox_info_idx['num'], bbox_info_idx['score'] ), \
            #                     (bbox_info_idx['loc'][0], bbox_info_idx['loc'][1] - 15), 
            #                     cv2.FONT_HERSHEY_COMPLEX, 3, color_dict["plate"], 2)
            ...
            img = cv_plot_rectangle(img, bbox_info_idx['loc'], mode=mode, color=color_dict["plate_capture"], thickness=3)
...
def draw_bbox_info_result_jpg(img, bbox_info):
    ...
    if len(bbox_info):
        ...
        img = cv2.putText(img, "{}_{}_{:.2f}".format( bbox_info_idx['column'], bbox_info_idx['num'], bbox_info_idx['score'] ), \
                            (0, 50), 
