...
class CaptureApi():
    ...
    def model_init(self):
        # detector
        if options.ssd_bool:
            self.detector = SSDDetector(prototxt=options.ssd_car_plate_prototxt, model_path=options.ssd_car_plate_model_path, ssd_caffe_bool=options.ssd_caffe_bool, ssd_openvino_bool=options.ssd_openvino_bool, merge_class_bool=options.car_attri_merge_bool, gpu_bool=options.gpu_bool)
        ...
        if options.lpr_type == "china":
            if options.lpr_caffe_bool:
                self.lpr = LPRCaffe(options.lpr_ocr_caffe_prototxt, options.lpr_ocr_caffe_model_path, input_shape=options.input_shape, ocr_labels=options.lpr_ocr_labels, prefix_beam_search_bool=options.lpr_ocr_prefix_beam_search_bool, gpu_bool=options.gpu_bool)
                self.lpr_seg = LPRSegColorClassCaffe(options.lpr_seg_caffe_prototxt, options.lpr_seg_caffe_model_path, options.lpr_seg_city_dict_name, options.lpr_seg_color_dict_name, input_shape=options.lpr_seg_input_shape, city_bool=options.lpr_seg_city_bool, color_bool=options.lpr_seg_color_bool, gpu_bool=options.gpu_bool)

            elif options.lpr_pytorch_bool:
                self.lpr = LPRPytorch(options.lpr_ocr_pth_path, input_shape=options.input_shape, ocr_labels=options.lpr_ocr_labels, prefix_beam_search_bool=options.lpr_ocr_prefix_beam_search_bool)
    ...
    def update_bbox_state_container(self, bbox_info_list):
        ...
        for idx in range(len(bbox_info_list)):
            ...
            for key, bbox_state_idy in self.params_dict['bbox_state_container'].items():
                ...
                if bbox_info_idx['track_id'] == bbox_state_idy['track_id']:
                    ...
                    if bool_add_lpr:                        
                        bbox_state_idy['plate_info']['disappear_frame_num'] = 0
                        bbox_state_idy['state']['lpr_num'] += 1
                        ...
                        bbox_state_idy['plate_info']['disappear_frame_num'] += 1
                    ...
                    bbox_info_idx['state']['lpr_num'] = bbox_state_idy['state']['lpr_num']                 
        ...
        for key, bbox_state_idy in self.params_dict['bbox_state_container'].items():
            ...
            bbox_state_idy['state']['center_point_list'].append( ( (bbox_state_idy['car_info']['roi'][0] + bbox_state_idy['car_info']['roi'][2]) / 2 , (bbox_state_idy['car_info']['roi'][1] + bbox_state_idy['car_info']['roi'][3]) / 2 ) )
    ...
    def update_capture_dict(self):
        ...
        for _, bbox_state_idy in self.params_dict['bbox_state_container'].items():
            ...
            if bbox_state_idy['plate_info']['disappear_frame_num'] == 0:
                ...
                if bbox_state_idy['state']['up_down_state'] == 'Near' and bbox_state_idy['state']['up_down_state_frame_num'] >= 3:
                    if abs(loc_center_y - options.Down_threshold) < options.capture_plate_up_down_distance_boundary_threshold and \
                        bbox_state_idy['state']['lpr_num'] > options.capture_plate_frame_threshold:
                        near_flage = True
                ...
                # 如果车辆在视野内，超过 50 帧
                if bbox_state_idy['state']['lpr_num'] > options.capture_outtime_frame_threshold_01:
                    outtime_flage_01 = True

                # 如果车辆在视野内，超过 150 帧
                if bbox_state_idy['state']['lpr_num'] > options.capture_outtime_frame_threshold_02:
    ...
    def update_capture_state(self):
        ...
        for capture_id_idx, capture_dict_idy in self.params_dict['capture_container'].items():
            ...
            capture_flage_idx = capture_dict_idy['capture']['flage']
