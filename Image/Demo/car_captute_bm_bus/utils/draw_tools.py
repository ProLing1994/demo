...
def draw_bbox_info(img, bbox_info, mode='xywh'):
    ...
    for idx in range(len(bbox_info)):
        ...
        img = cv2.putText(img, "{}_{}_{}_{}_{}_{}_{:.1f}_{:.1f}_{:.1f}_{:.1f}_{:.1f}".format( bbox_info_idx['id'], bbox_info_idx['frame_num'], bbox_info_idx['up_down_state'], bbox_info_idx['up_down_state_frame_num'], bbox_info_idx['left_right_state'], bbox_info_idx['left_right_state_frame_num'], bbox_info_idx['up_down_speed'], bbox_info_idx['left_right_speed'], bbox_info_idx['dist_capture_line_left'], bbox_info_idx['dist_capture_line_right'], bbox_info_idx['dist_capture_line_up'] ), (bbox_info_idx['loc'][0], bbox_info_idx['loc'][1] - 10), 
                            cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 2)
        img = cv2.putText(img, "{}_{}_{}_{}_{}_{}_{}_{}".format( int(bbox_info_idx['left_warning_state']), int(bbox_info_idx['left_alarm_flage']), int(bbox_info_idx['right_warning_state']), int(bbox_info_idx['right_alarm_flage']), int(bbox_info_idx['up_warning_state']), int(bbox_info_idx['up_alarm_flage']), int(bbox_info_idx['warning_flage']), int(bbox_info_idx['alarm_flage'])), (bbox_info_idx['loc'][0], bbox_info_idx['loc'][1] + 20), 
                            cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 2)
