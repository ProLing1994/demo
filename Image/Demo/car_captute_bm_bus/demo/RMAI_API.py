...
class CaptureApi():
    ...
    def option_init(self):
        ...
        self.yolov6_checkpoint = "/mnt/huanyuan/model/image/yolov6/yolov6_c28_car_0214/epoch_10.pth"
    ...
    def param_init(self):
        ...
        bbox_info_dict['dist_capture_line_up'] = 0                          # 距离抓拍线位置
        ...
        bbox_info_dict['up_warning_state'] = 0                              # 抓拍标志位
        ...
        bbox_state_dict['dist_capture_line_up'] = 0                         # 距离抓拍线位置
        ...
        bbox_state_dict['up_warning_state'] = 0                             # 抓拍标志位
        ...
        for idx in range(len(bbox_info_list)):
            ...
            for key, bbox_state_idy in self.params_dict['bbox_state_container'].items():
                ...
                if bbox_info_idx['id'] == bbox_state_idy['id']:
                    ...
                    # dist
                    bbox_state_idy['dist_capture_line_up'] = (-1) * ( car_center_x * (capture_line_top_k + 1e-5) + capture_line_top_b - car_bottom_y )
                    ...
                    bbox_info_idx['dist_capture_line_up'] = bbox_state_idy['dist_capture_line_up']
                    ...
                    bbox_info_idx['up_warning_state'] = bbox_state_idy['up_warning_state']
    ...
    def update_capture(self):
        ...
        for _, bbox_state_idy in self.params_dict['bbox_state_container'].items():
            ...
            # 如果车辆向近处行驶/向远处行驶
            # 对车辆运行轨迹不敏感：因摄像头畸变，对上方远处车辆行驶轨迹判断不敏感
            # up_warning / up_alarm
            if (car_right_x > capture_point_left_top[0] and car_left_x < capture_point_right_top[0]):
                if bbox_state_idy['dist_capture_line_up'] > ( -1 * self.capture_up_alarm[1]) and \
                    bbox_state_idy['dist_capture_line_up'] < ( -1 * self.capture_up_alarm[0]) : 

                    if bbox_state_idy['up_warning_state'] == 2:
                        bbox_state_idy['up_alarm_flage'] = True 
                    else:
                        bbox_state_idy['up_warning_state'] = 1
            
                if bbox_state_idy['dist_capture_line_up'] > self.capture_up_alarm[0] and \
                    bbox_state_idy['dist_capture_line_up'] < self.capture_up_alarm[1] :

                    if bbox_state_idy['up_warning_state'] == 1: 
                        bbox_state_idy['up_alarm_flage'] = True 
                    else:
                        bbox_state_idy['up_warning_state'] = 2
            ...
            # alarm_flage
            if (bbox_state_idy['left_alarm_flage'] and bbox_state_idy['right_alarm_flage']) or \
