...
def _linear(raw, input, weight, bias=None):
    ...
    layer = caffe_net.Layer_param(
        name=layer_name, type="InnerProduct", bottom=[log.blobs(input)], top=top_blobs
    )
    if len(x.size()) == 1:
        layer.fc_param(1, has_bias=bias is not None)
    else:
...
def _split(raw, tensor, split_size, dim=0):
    ...
    if isinstance(split_size, list) or isinstance(split_size, tuple):
        split_point = [split_size[0]]
        for i in range(1, len(split_size) - 1):
            split_point.append(split_size[i] + split_point[-1])
        slice_param = caffe_net.pb.SliceParameter(
            axis=dim, slice_point=split_point
        )
    else:
...
# upsample layer
def _interpolate(
    raw, input, size=None, scale_factor=None, mode="nearest", align_corners=None
):
    ...
    if mode == "bilinear" and align_corners == True:
        ...
        layer = caffe_net.Layer_param(
            name=name, type="Interp", bottom=[log.blobs(input)], top=[log.blobs(x)]
        )
    ...
    layer = caffe_net.Layer_param(
        name=layer_name, type="Upsample", bottom=[log.blobs(input)], top=top_blobs
    )
    ...
    layer.upsample_param(size=None, scale_factor=scale_factor)
...
def _permute(input, *args):
    ...
    layer = caffe_net.Layer_param(
        name=layer_name, type="Permute", bottom=[log.blobs(input)], top=top_blobs
    )
...
def _mean(input, *args, **kwargs):
    ...
    layer = caffe_net.Layer_param(
        name=layer_name, type="Reduction", bottom=[log.blobs(input)], top=top_blobs
    )
    if len(args) == 1:
        dim = args[0]
    elif "dim" in kwargs:
...
def trans_net(net, input_var, name="TransferedPytorchModel"):
    ...
    if hasattr(net, 'forward_caffe'):
        out = net.forward_caffe(input_var)
    elif hasattr(net, 'forward_dummy'):
        out = net.forward_dummy(input_var)
    else:
        out = net(input_var)
