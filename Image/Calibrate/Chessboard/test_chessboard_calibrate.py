...
def test_solvePnP(args):
    ...
    cp_world = np.zeros((w * h, 3), dtype = np.float32)
    cp_img = np.zeros((w * h, 1, 2), dtype = np.float32)
    for track in annotation['shapes']:
        label = track['label']
        points = np.array(track['points'])

        for idx in range(len(args.chessboard_corner_name_list)):
            chessboard_corner_name = args.chessboard_corner_name_list[idx]

            if label != chessboard_corner_name:
                continue
            
            # cp_world_idx
            w = args.chessboard_corner_shape[0]
            h = 1
            cp_int_idx = np.zeros((w * h, 3), np.float32)
            cp_int_idx[:, :2] = np.mgrid[0:w, 0:h].T.reshape(-1, 2)
            cp_world_idx = cp_int_idx * args.chessboard_size_per_grid
            # x 偏移
            cp_world_idx = cp_world_idx - np.array([2, 0 ,0])
            # y 偏移
            cp_world_idx = cp_world_idx + np.array([0, idx-0.1, 0])
            # cp_world_idx = cp_world_idx + np.array([0, idx+1, 0])

            # cp_world
            cp_world[(idx)*w: (idx+1)*w] = cp_world_idx
            # print(cp_world_idx)
            
            # cp_img
            cp_img[(idx)*w: (idx+1)*w] = points.reshape((w,1,2))
            # print(points.reshape((w,1,2)))
...
if __name__ == '__main__':
    ...
    args.chessboard_corner_name_list = ['c1', 'c2', 'c3', 'c4', 'c5']
    args.calibrate_corner_name_list = ['b5_0', 'b5_10', 'b0_10', 'b-5_10', 'b-5_0']
    args.feet_corner_name_list = [ ['b6_0', 'b6_5', 'b6_10', 'b-6_0', 'b-6_5', 'b-6_10'], 
    ...
    args.test_chessboard_img_path = os.path.join(args.file_dir, "test_jpg/chessboard_img/{}.jpg".format(args.image_name))
    args.test_chessboard_json_path = os.path.join(args.file_dir, "test_jpg/chessboard_img/{}.json".format(args.image_name))

    # 利用棋盘格标定板，推到外参
    img, rvecs, tvecs, cameraMatrix, distCoeffs = test_solvePnP(args)
    output_test_solvePnP_img_path = os.path.join(args.file_dir, "test_jpg/chessboard_img_res/{}_solvePnP.jpg".format(args.image_name))
    ...
    output_test_solvePnP_calibrate_corner_img_path = os.path.join(args.file_dir, "test_jpg/chessboard_img_res/{}_solvePnP_calibrate_corner_{:.2f}.jpg".format(args.image_name, error))
    ...
    for idx in range(len(args.feet_corner_name_list)):
        ...
        output_test_solvePnP_feet_corner_img_path = os.path.join(args.file_dir, "test_jpg/chessboard_img_res/{}_solvePnP_feet_corner_{}_{:.2f}.jpg".format(args.image_name, idx, error))
