...
def GetMfscFeature(wavsignal, fs):
    if(fs==8000):
        nfft=256
    else:
        nfft=512
    wav_feature = mfsc(wavsignal, fs, numcep=64, winlen=0.032, winstep=0.010, nfft=nfft, nfilt=64, lowfreq=10, highfreq=None,
             appendEnergy=False)
    wav_feature=np.log(1+wav_feature)
    return wav_feature
...
def mfsc(signal, samplerate=16000, winlen=0.025,winstep=0.01, numcep=13, nfilt=26,nfft=512, lowfreq=0,highfreq=None,
...
def fbank2(signal,samplerate=16000,winlen=0.025,winstep=0.01,
    ...
    '''
    energy=[]
    pspec=GetFrequencyFeature3(signal,samplerate)
